...
# How many LoRA adapters to keep resident. 4 covers narration alternating a
# couple of characters without reloads, costs ~500 MiB, and stays workable on
# the 8 GB GPUs the README asks for. Reloading an evicted adapter from disk is
# ~1-2 s, against certain OOM if they accumulate.
_MAX_CACHED_LORA_ADAPTERS = 4
...
class InferenceRuntime:
    def __init__(
        ...
        *,
        ...
        # Loaded adapters, oldest-used first. The worker process is long-lived
        # now, and each adapter holds ~124 MiB of VRAM that PEFT never frees on
        # its own — cycling through 80 characters in one session grows the
        # process by ~10 GB (measured +2.3 GiB over 20 adapters) and OOMs an
        # 8 GB GPU after 10-15 switches. Keep only the most recent few and
        # delete_adapter() the rest.
        self._lora_adapter_names: "OrderedDict[str, str]" = OrderedDict()
    ...
    def _prepare_lora_for_request_inner(
        ...
        *,
        ...
        if adapter_name is None:
            ...
            messages.append(msg)
            log_fn(msg)
            self._evict_lora_adapters(log_fn=log_fn)
        else:
            # Mark as most recently used so it is not the next eviction victim.
            self._lora_adapter_names.move_to_end(resolved_path)
    ...
    def _evict_lora_adapters(self, *, log_fn: Callable[[str], None]) -> None:
        """Drop the least recently used adapters to make room for one more.

        Runs before a new adapter loads, so keeping len < cap leaves exactly
        one free slot. Deleting the currently active adapter is fine: the
        caller activates the new one via set_adapter immediately after.
        """
        delete_adapter = getattr(self.model, "delete_adapter", None)
        if not callable(delete_adapter):
            return
        evicted = False
        while len(self._lora_adapter_names) >= _MAX_CACHED_LORA_ADAPTERS:
            old_path, old_name = next(iter(self._lora_adapter_names.items()))
            try:
                delete_adapter(old_name)
            except Exception as exc:  # noqa: BLE001 - eviction must not kill the request
                # 台帳から先に消すと、PEFT 側に実体が残ったまま上限の勘定が
                # 狂い、失敗が続く限り実体数が際限なく増える。台帳は実体と
                # 一致させたままこの回は諦め、次のロード時に再試行する。
                log_fn(f"warn: failed to evict LoRA adapter {old_path}: {exc}")
                break
            del self._lora_adapter_names[old_path]
            evicted = True
            log_fn(f"info: evicted LoRA adapter to cap VRAM: {old_path}")
        if evicted and self.model_device.type == "cuda":
            # delete_adapter frees the tensors; give the blocks back to the
            # allocator pool so the drop is visible outside this process too.
...
def _load_audio(path: str | Path) -> tuple[torch.Tensor, int]:
    # 実体は codec 側。torchaudio が使えるかの判定を1度だけ行い、
    # 以降は使い回す（毎回試すと1件あたり十数ミリ秒を捨てる）。
...
def save_wav(path: str | Path, audio: torch.Tensor, sample_rate: int) -> Path:
    ...
    # 書き出しも読み込みと同じ torchcodec バックエンドを使うので、判定を共有する。
    # 生成のたびに失敗する呼び出しを繰り返さない。
